...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	$f_role = gpc_get_int('role');
	$f_agency = gpc_get_int('agency');
	$f_unit_department = gpc_get_int('unit_department');
	...
	$t_old_role = $t_user['role'];
	$t_old_agency = $t_user['agency'];
	$t_old_unit_department = $t_user['unit_department'];
	...
	$c_role =  $f_role ;
	$c_agency =  $f_agency ;
	$c_unit_department = $f_unit_department;
	...
	if ( $f_protected && $t_old_protected ) {
	    ...
	    $query = "UPDATE $t_user_table
	    		SET username=" . db_param() . ", email=" . db_param() . ",
	    			...
	    			protected=" . db_param() . ", realname=" . db_param() . ", role=" . db_param() . ",agency=".db_param().",unit_department=".db_param()."
	    		WHERE id=" . db_param();
	    $query_params = Array( $c_username, $c_email, $c_access_level, $c_enabled, $c_protected, $c_realname, $c_role, $c_agency, $c_unit_department, $c_user_id );
	}
	...
	if ( $f_send_email_notification ) {
		...
		if (strcmp( $f_role, $t_old_role)){
			$t_old_role_string = get_enum_element ( 'user_roles', $t_old_role );
			$t_new_role_string = get_enum_element ( 'user_roles', $f_role);
			$t_changes .= lang_get( 'role' ) . ': ' . $t_old_role_string . ' => ' . $t_new_role_string . "\n\n";
		}
		if (strcmp( $f_agency, $t_old_agency)){
			$t_old_agency_string = get_enum_element ( 'btl_agencies', $t_old_agency );
			$t_new_agency_string = get_enum_element ( 'btl_agencies', $f_agency);
			$t_changes .= lang_get( 'agency' ) . ': ' . $t_old_agency_string . ' => ' . $t_new_agency_string . "\n\n";
		}
		if (strcmp( $f_unit_department, $t_unit_department)){
			$t_old_unit_department_string = get_enum_element ( 'btl_units_departments', $t_old_unit_department );
			$t_new_unit_department_string = get_enum_element ( 'btl_units_departments', $f_unit_department);
			$t_changes .= lang_get( 'unit_department' ) . ': ' . $t_old_unit_department_string . ' => ' . $t_new_unit_department_string . "\n\n";
		}
