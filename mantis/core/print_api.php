...
function get_user_roles_option_list( $p_user_auth = 0, $p_current_value = 0, $p_show_current = true,  $p_project_id = ALL_PROJECTS ) {
	$t_config_var_value = config_get( 'user_roles_enum_string', null, null, $p_project_id );
	...
	foreach ( $t_enum_values as $t_enum_value ) {
		if (   ( $p_show_current || $p_current_value != $t_enum_value )
			...
			$t_enum_list[$t_enum_value] = get_enum_element( 'role', $t_enum_value );
		}
	}

	if ( $p_show_current ) {
		$t_enum_list[$p_current_value] = get_enum_element( 'role', $p_current_value );
	}

	return $t_enum_list;
}

function print_user_roles_option_list( $p_select_label, $p_current_value = 0, /*parametre à enlever $p_allow_close = false,*/ $p_project_id = ALL_PROJECTS ) {
	$t_current_auth = access_get_project_level( $p_project_id );

	$t_enum_list = get_user_roles_option_list( $t_current_auth, $p_current_value, /*true,*/ $p_allow_close, $p_project_id );
	...
	if( count( $t_enum_list ) > 1 ) {
		...
		echo MantisEnum::getLabel( lang_get( 'user_roles_enum_string' ), $p_current_value );
	}
}
