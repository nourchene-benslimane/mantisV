...
<?php
	...
	$t_columns = array(
		...
		'enabled', 'protected', 'date_created', 'last_visit','role','agency','unit_department'
	);
...
<?php
	...
	for ($i=0;$i<$user_count;$i++) {
		...
		if( !isset( $t_role[$u_role] ) ) {
			$t_role[$u_role] = get_enum_element( 'user_roles', $u_role );
		}
		if( !isset( $t_agency[$u_agency] ) ) {
			$t_agency[$u_agency] = get_enum_element( 'btl_agencies', $u_agency );
		}
		if( !isset( $t_unit_department[$u_unit_department] ) ) {
			$t_unit_department[$u_unit_department] = get_enum_element( 'btl_units_departments', $u_unit_department );
		}
?>
<tr <?php echo helper_alternate_class( $i ) ?>>
	...
	<td> <?php echo $t_role[$u_role] ?> </td>
	<td> <?php if ($u_agency != 0) {echo $t_agency[$u_agency];} ?> </td>
	<td> <?php if ( $u_unit_department !=0 ) { echo $t_unit_department[$u_unit_department]; }?> </td>
</tr>
<?php
	}  # end for
